  ...
  const { targetRef, tooltip, tooltipVisible } = useTooltip(
    <>
      ...
      {strikethrough && (
        <Text color="textSubtle">
          {t('Available Boosted')}:{' '}
          <Text color="textSubtle" style={{ display: 'inline-block' }}>
            {t('Up to %boostMultiplier%x', { boostMultiplier: boostMultiplierDisplay })}
          </Text>
        </Text>
      )}
      {strikethrough && <Text color="textSubtle">{t('Boost only applies to base APR (CAKE yield)')}</Text>}
    </>,
    {
      placement: 'top',
    },
